...
start_ambient() {
    if ! pgrep -f "mpv.*$MPV_IPC" > /dev/null; then
        mpv --input-ipc-server=$MPV_IPC --loop --no-video --volume=60 /home/luno/Audios/Background.mp3 &
        AMBIENT_PLAYER_PID=$!
...
other_audio_active() {
    # Corrected: Use .[] instead of .nodes[]
    active=$(pw-dump | jq '.[] | select(.type == "PipeWire:Interface:Node" and .info.props["media.class"] == "Audio/Source" and .info.props["application.name"] != "mpv") | .info.props["node.name"]')
    [[ -n "$active" ]]
}
