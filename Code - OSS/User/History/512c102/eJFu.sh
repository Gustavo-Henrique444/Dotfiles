...
while true; do
    ...
    if [[ "$url" == *"youtube.com"* ]]; then
     ...
     if [[ "$url" == *"reddit.com"* ]]; then
        if [[ "$status" == "Playing" && "$last_state" != "playing" ]]; then
            echo "[+] reddit is playing: $url"
            fade_out
            last_state="playing"
        elif [[ "$status" != "Playing" && "$last_state" == "playing" ]]; then
            echo "[+] reddit paused/stopped: $url"
            fade_in
            last_state="paused"
        fi
    else
        if [[ "$last_state" == "playing" ]]; then
            echo "[+] reddit closed or switched away"
            fade_in
            last_state="stopped"
        fi
    fi

     if [[ "$url" == *"twitch.com"* ]]; then
        if [[ "$status" == "Playing" && "$last_state" != "playing" ]]; then
            echo "[+] twitch is playing: $url"
            fade_out
            last_state="playing"
        elif [[ "$status" != "Playing" && "$last_state" == "playing" ]]; then
            echo "[+] twitch paused/stopped: $url"
            fade_in
            last_state="paused"
        fi
    else
        if [[ "$last_state" == "playing" ]]; then
            echo "[+] twitch closed or switched away"
            fade_in
            last_state="stopped"
        fi
    fi
